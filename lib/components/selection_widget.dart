...
import 'package:dropdown_search/src/widgets/checkbox_widget.dart';

import 'button.dart';
import 'dropdown_search.dart';
import 'flutter_simple_treeview/flutter_simple_treeview.dart';
...
class SelectionWidgetState<T> extends State<SelectionWidget<T>> {
  ...
  Widget _multiSelectionValidation() {
    ...
    Widget defaultValidation = Expanded(
      child: Container(
  ...
  List<TreeNode> _getChildList(List<Map<String, dynamic>> items) {
    return items.map((_item) {
      ...
      if (widget.isMultiSelectionMode) {
        w = CheckBoxWidget(
          layout: (context, isChecked) => Padding(
            padding: EdgeInsets.symmetric(vertical: 4),
            child: Text(_item['text']),
          ),
      ...
      if (_item.containsKey('children')) {
        List<dynamic> ch = _item['children'];
        List<Map<String, dynamic>> children =
            ch.map((e) => e as Map<String, dynamic>).toList();
        return TreeNode(
          content: Expanded(
            child: InkWell(
              child: w,
          ...
          children: _getChildList(children),
        );
      }
      return TreeNode(
        content: Expanded(
